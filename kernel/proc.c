...
getptable(int nproc, char *buffer)
{
  ...
  // Copy info[] to user-provided buffer using copyout
  int size = count * sizeof(struct uproc);
  if (copyout(myproc()->pagetable, (uint64)buffer, (char *)info, size) < 0)
    return 0;
